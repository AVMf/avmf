...
public class GenerationObject extends ObjectiveFunction {
	...
	public void FillSetofSolutionsOfExistingBehaviourPairs(List<BehaviourPair> ExistingBehaviourPairs){
	...
	public Vector setUpVector(int size) {
		...
		//source state label
		vector.addVariable(new IntegerVariable(0, 0, this.existingstatemachine.getallstates().size() - 1));
	...
	public ObjectiveValue computeObjectiveValue(Vector v) {
		...
		//first, the similarity of the current solution with the existing behavior pairs which are from state machine.
		...
		for(int i = 0; i < len2; i ++){
			result_diversity = result_diversity + DiversityOfTwoVectors(
		...
		result = max_min_normalization(result_diversity, 0, 3) + max_min_normalization(result_similarity, 0, 3);
		result = 1 - result/2;
	...
	public double DiversityOfTwoVectors(Vector v1, Solution v2){//Distance of two solutions
		...
		//v1
		//source state
		State sourceState_v1 = this.existingstatemachine.getallstates().get( Integer.parseInt( v1.getVariable(0).toString()));
		//source state v2
		...
		//Eculidean Metric
		double result = 0;
		result = Math.pow(normalization(distance_source_ac), 2)
	...
	public double SimilarityBetweenVectorAndTransition(Vector v1, Transition t1){//9 variables in space
		
		double result = 3 - DistanceBetweenVectorAndTransition(v1, t1);
		
		return result;
	}
	public double DistanceBetweenVectorAndTransition(Vector v1, Transition t1){//Distance of two solutions
		...
		double result = 0;
		result = Math.pow(nor_distance_source_ac, 2)
		...
		if(packetloss_v1 < 10 || packetdelay_v1 < 1 || packetduplication_v1 < 1 || packetcorruption_v1 < 1) {
			return 3.0;
		}
		if(distance_packetloss < 1 || distance_packetdelay < 1 || distance_packetduplication < 1 || distance_packetcorruption < 1) {
			return 3.0;
		}
		if(distance_packetloss + distance_packetdelay + distance_packetduplication + distance_packetcorruption < 20) {
			return 3.0;
		}
