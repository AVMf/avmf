...
public class GenerateInputData {
    ...
    static class GenerateInputDataArgsParser extends ArgsParser {
        static final int TEST_OBJECT_PARAM_INDEX = 0, BRANCH_PARAM_INDEX = 1, SEARCH_PARAM_INDEX = 2, MIN_NUM_ARGS = 2;
        ...
        protected void addParams() {
            addParam("testobject", "a test object to generate data for (e.g., \"Calendar\", \"Line\" or \"Triangle\")");
            addParam("branch", "a branch ID of the form X(T|F) where X is a branching node number (e.g., \"5T\")");
            super.addParams();
        }
        ...
        void wrongNumberOfArgumentsError() {
            error("Wrong number of arguments -- expected at least " + MIN_NUM_ARGS + " got " + args.length);
        }
    }
}
